...
/**
 * Regression for stakeholder report: looking up a White Island seal while the device is also
 * at White Island caused the Population row to disappear with no error. Lookup still returned
 * population = "White Island"; [LookupCard] only rendered Population inside the "outside White
 * Island" warning branch and had no else when the auto-detected colony matched.
 */
...
class LookupCardWhiteIslandPopulationTest {
    ...
    @Test
    fun nonWhiteIslandSeal_alwaysShowsPopulation() {
        homeViewModel.setAutoDetectedColony(
            TestFixtures.sampleColony(location = "White Island"),
        )
